...
namespace bemtool
{
    ...
    public:
        ...
        template <typename m_t>
        int Push(e_t e, const m_t& m, const int& j)
        {
    ...
    public:
        ...
        Geometry(const f_t& f) : mesh_file(f) { (*this).LoadNodes(); }

        Geometry(const std::vector<v_t>&);

        Geometry()
        {
        }; // add default constructor AF 07/19
        ...
        template <class i_t>
        const subarray<const Geometry, i_t> operator[](const i_t& i_) const
        {
            return subarray<const Geometry, i_t>(*this, i_);
        }

        template <typename m_t>
        int Push(Elt1D e, const m_t& m, const int& j) { return elt1D.Push(e, m, j); }

        template <typename m_t>
        int Push(Elt2D e, const m_t& m, const int& j) { return elt2D.Push(e, m, j); }

        template <typename m_t>
        int Push(Elt3D e, const m_t& m, const int& j) { return elt3D.Push(e, m, j); }
    ...
    struct GetElt
    {
        static inline const std::vector<Elt<D>>& Of(const Geometry& g);
    };

    template <>
    inline const std::vector<Elt1D>& GetElt<1>::Of(const Geometry& g) { return Elt1DOf(g); }

    template <>
    inline const std::vector<Elt2D>& GetElt<2>::Of(const Geometry& g) { return Elt2DOf(g); }

    template <>
    inline const std::vector<Elt3D>& GetElt<3>::Of(const Geometry& g) { return Elt3DOf(g); }
    ...
    public:
        ...
        void Load(Geometry& g, int ref = -1)
        {
        ...
        inline void set_elt(Geometry& g)
        {
            node = &g;
            elt = &GetElt<D>::Of(g);
        }
